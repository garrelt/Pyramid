module analytical_column_density
  ...
  use array, only: analytical_xHI, analytical_xHeI, &
				   ...
				   analytical_HI_column_density_in, analytical_HeI_column_density_in, &
				   analytical_HeII_column_density_in, analytical_HI_column_density_out, &
				   analytical_HeI_column_density_out, analytical_HeII_column_density_out, &
				   analytical_shell_volume 
				   
  contains
	
  subroutine analytical_column_density_calculation
      ...
      do j = 1,2*level_py
        do k = 1,2*level_py
		  ...
		  if (absolute_x.eq.1 .and. absolute_y.eq.1 .and. absolute_z.eq.1) then
  		    analytical_HI_column_density_in(i,j,k) = 0.0
            analytical_HeI_column_density_in(i,j,k) = 0.0
  		    analytical_HeII_column_density_in(i,j,k) = 0.0
            analytical_HI_column_density_out(i,j,k) = cellsize_py*analytical_number_density*analytical_xHI
  		    analytical_HeI_column_density_out(i,j,k) = cellsize_py*analytical_number_density*analytical_xHeI
            analytical_HeII_column_density_out(i,j,k) = cellsize_py*analytical_number_density*analytical_xHeII
  		    analytical_shell_volume(i,j,k) = 8.0*cellsize_py*cellsize_py*cellsize_py
          else									 
		    ...
		    analytical_HI_column_density_in(i,j,k) = in_distance*analytical_number_density*analytical_xHI
            analytical_HeI_column_density_in(i,j,k) = in_distance*analytical_number_density*analytical_xHeI
		    analytical_HeII_column_density_in(i,j,k) = in_distance*analytical_number_density*analytical_xHeII
            analytical_HI_column_density_out(i,j,k) = out_distance*analytical_number_density*analytical_xHI
		    analytical_HeI_column_density_out(i,j,k) = out_distance*analytical_number_density*analytical_xHeI
            analytical_HeII_column_density_out(i,j,k) = out_distance*analytical_number_density*analytical_xHeII
		    analytical_shell_volume(i,j,k) = four_over_three_pi*(out_distance*out_distance*out_distance-&
  ...
  end subroutine analytical_column_density_calculation	
	
end module analytical_column_density