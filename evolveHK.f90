...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine short_characteristic (ray_traced_pos,srcpos,cdensi,path)
    ...
    integer :: i_minus,j_minus,k_minus
    integer :: i_global,j_global,k_global
    integer :: i_minus_global,j_minus_global,k_minus_global
    ...
    if (kdela >= jdela.and.kdela >= idela) then
       ...
       alam = (real(k_minus-k0)+sgnk*0.5)/dk
              
       xc = alam*di+real(i0) ! x of crossing point on z-plane 
       yc = alam*dj+real(j0) ! y of crossing point on z-plane
       
       dx = 2.0*abs(xc-(real(i_minus)+0.5*sgni)) ! distances from c-point to
       dy = 2.0*abs(yc-(real(j_minus)+0.5*sgnj)) ! the corners.
       ...
       i_global = modulo(i-1,mesh(1))+1
       i_minus_global = modulo(i_minus-1,mesh(1))+1
       j_global = modulo(j-1,mesh(2))+1
       j_minus_global = modulo(j_minus-1,mesh(2))+1
       k_minus_global = modulo(k_minus-1,mesh(3))+1
       c1 = coldensh_out(i_minus_global,j_minus_global,k_minus_global)    !# column densities at the
       c2 = coldensh_out(i_global,j_minus_global,k_minus_global)     !# four corners
       c3 = coldensh_out(i_minus_global,j_global,k_minus_global)
       c4 = coldensh_out(i_global,j_global,k_minus_global)
...
!endif
    ...
    elseif (jdela >= idela.and.jdela >= kdela) then
       ...
       alam = (real(j_minus-j0)+sgnj*0.5)/dj
       zc = alam*dk+real(k0)
       xc = alam*di+real(i0)
       dz = 2.0*abs(zc-(real(k_minus)+0.5*sgnk))
       dx = 2.0*abs(xc-(real(i_minus)+0.5*sgni))
       s1 = (1.0-dx)*(1.0-dz)
       s2 = (1.0-dz)*dx
       s3 = (1.0-dx)*dz
       s4 = dx*dz
       ...
       i_minus_global = modulo(i_minus-1,mesh(1))+1
       j_minus_global = modulo(j_minus-1,mesh(2))+1
       k_global = modulo(k-1,mesh(3))+1
       k_minus_global = modulo(k_minus-1,mesh(3))+1
       c1 = coldensh_out(i_minus_global,j_minus_global,k_minus_global)
       c2 = coldensh_out(i_global,j_minus_global,k_minus_global)
       c3 = coldensh_out(i_minus_global,j_minus_global,k_global)
       c4 = coldensh_out(i_global,j_minus_global,k_global)
...
!endif
    ...
    elseif(idela >= jdela.and.idela >= kdela) then
       ...
       alam = (real(i_minus-i0)+sgni*0.5)/di
       zc = alam*dk+real(k0)
       yc = alam*dj+real(j0)
       dz = 2.0*abs(zc-(real(k_minus)+0.5*sgnk))
       dy = 2.0*abs(yc-(real(j_minus)+0.5*sgnj))
       s1 = (1.0-dz)*(1.0-dy)
       s2 = (1.0-dz)*dy
       s3 = (1.0-dy)*dz
       s4 = dy*dz
       ...
       k_minus_global = modulo(k_minus-1,mesh(3))+1
       c1 = coldensh_out(i_minus_global,j_minus_global,k_minus_global)
       c2 = coldensh_out(i_minus_global,j_global,k_minus_global)
       c3 = coldensh_out(i_minus_global,j_minus_global,k_global)
       c4 = coldensh_out(i_minus_global,j_global,k_global)
       ! extra weights for better fit to analytical solution
       w1 = s1*weight_function(c1)
       w2 = s2*weight_function(c2)
       w3 = s3*weight_function(c3)
